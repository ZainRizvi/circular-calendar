...
export class Arc {
   ...
   */
  drawnPath(stroke: string = 'black', fill: string = 'none'): string {
    return `<path d="${this.path(ArcDrawMode.NEW)}" stroke="${stroke}" stroke-width="${STROKE_WIDTH}" fill="${fill}"/>`;
  }
}
...
export class DimensionalArc {
   ...
   */
  drawnPath(): string {
    return `<path d="${this.path()}" stroke="${this.stroke}" stroke-width="${STROKE_WIDTH}" fill="${this.fill}"/>`;
  }
}
