...
export function generateCircleCoverSvg(
  ...
  // The center after transform is at approximately (75 + widthCenter*0.3, 50 + (outermostRadius + verticalOffset)*0.3)
  const scaleFactor = 0.3;
  const translateX = 75;
  const translateY = 50;
  const scaledRadius = outermostRadius * scaleFactor;
  const centerX = translateX + widthCenter * scaleFactor;
  const centerY = translateY + (outermostRadius + verticalOffset) * scaleFactor;
